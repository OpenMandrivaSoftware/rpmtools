...
#     CCFLAGS => q[-Wall]
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[gendistrib], q[genhdlist2], q[packdrake], q[dumpdistribconf]]
#     INC => q[]
#     INSTALLDIRS => q[vendor]
#     LIBS => [q[]]
#     MAN1PODS => { dumpdistribconf=>q[$(INST_MAN1DIR)/dumpdistribconf.1], genhdlist2=>q[$(INST_MAN1DIR)/genhdlist2.1], packdrake=>q[$(INST_MAN1DIR)/packdrake.1], gendistrib=>q[$(INST_MAN1DIR)/gendistrib.1] }
#     MAN3PODS => {  }
#     NAME => q[rpmtools]
#     OBJECT => q[]
#     PM => { packdrake.pm=>q[$(INST_LIBDIR)/packdrake.pm] }
#     PREREQ_PM => {  }
#     VERSION => q[7.00.2]
#     depend => { clean_subdirs=>q[cleanc] }
#     dist => { COMPRESS=>q[xz -f --best --text], SUFFIX=>q[.xz] }
#     macro => { RPMTOOLSVERSION=>q[7.00.2] }
...
CCCDLFLAGS = -fno-PIE -fPIC
CCDLFLAGS = -fno-PIE -Wl,--no-undefined -Wl,--warn-unresolved-symbols
DLEXT = so
DLSRC = dl_dlopen.xs
EXE_EXT = 
FULL_AR = /usr/bin/ar
LD = x86_64-mandriva-linux-gnu-gcc
LDDLFLAGS = -shared -fno-PIE -O2 -Wa,--compress-debug-sections -gdwarf-4 -fvar-tracking-assignments -frecord-gcc-switches -Wstrict-aliasing=2 -pipe -Wformat -Werror=format-security -Wp,-D_FORTIFY_SOURCE=2 -fstack-protector --param=ssp-buffer-size=4 -fPIC -Wl,--no-undefined -Wl,--warn-unresolved-symbols
LDFLAGS =  -Wl,--no-undefined  -fstack-protector
LIBC = /lib/libc-2.19.so
LIB_EXT = .a
OBJ_EXT = .o
OSNAME = linux
OSVERS = 3.11.0-9-generic
RANLIB = :
...
TO_INST_PM = packdrake.pm

PM_TO_BLIB = packdrake.pm \
...
DISTNAME = rpmtools
DISTVNAME = rpmtools-7.00.2


# --- MakeMaker macro section:
RPMTOOLSVERSION = 7.00.2
...
EXE_FILES = gendistrib genhdlist2 packdrake dumpdistribconf

pure_all :: $(INST_SCRIPT)/dumpdistribconf $(INST_SCRIPT)/genhdlist2 $(INST_SCRIPT)/packdrake $(INST_SCRIPT)/gendistrib
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/dumpdistribconf $(INST_SCRIPT)/genhdlist2 \
	  $(INST_SCRIPT)/packdrake $(INST_SCRIPT)/gendistrib 

$(INST_SCRIPT)/dumpdistribconf : dumpdistribconf $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/dumpdistribconf
	$(CP) dumpdistribconf $(INST_SCRIPT)/dumpdistribconf
	$(FIXIN) $(INST_SCRIPT)/dumpdistribconf
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/dumpdistribconf

$(INST_SCRIPT)/genhdlist2 : genhdlist2 $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/genhdlist2
	$(CP) genhdlist2 $(INST_SCRIPT)/genhdlist2
	$(FIXIN) $(INST_SCRIPT)/genhdlist2
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/genhdlist2

$(INST_SCRIPT)/packdrake : packdrake $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
...
$(INST_SCRIPT)/gendistrib : gendistrib $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: rpmtools' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires: {}' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: v7.00.2' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "rpmtools",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "v7.00.2"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
